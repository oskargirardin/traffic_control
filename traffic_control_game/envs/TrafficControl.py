...
class TrafficControlEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.ps = self.np_random.uniform(low=0.05, high=0.5, size=len(self.dirs))  # probabilities of car generation for each line??
    ...
    def step(self, action):
        ...
        if self.game.check_crash():
            reward = -100  # negative rewards for termination??
            terminated = True
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_LEFT:
                        self.game.switch_light("right")
                    if event.key == pygame.K_UP:
                        self.game.switch_light("down")
                    if event.key == pygame.K_RIGHT:
                        self.game.switch_light("left")
                    if event.key == pygame.K_DOWN:
                        self.game.switch_light("up")
